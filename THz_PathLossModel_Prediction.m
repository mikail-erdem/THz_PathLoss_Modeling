clc;clear;
load('THz_PathLossModeling_Parameters_TransmittanceData.mat')
[ff,dd,hh] = meshgrid(f_c,d_c,h_c);
...
transOrig = interp3(ff,dd,hh,trans_c,f,d,h);

disp(['Trans-True Value: ',num2str(transOrig)])
disp(['Trans-Pred Value: ',num2str(transPred)])
disp(['Trans-Error Value: ',num2str(abs(transPred-transOrig))])

%% Calculate pathloss errors
% Transmittance
[PLspread_dB_Pred , PLabs_dB_Pred] = Trans2PL(f,d,transPred);
[PLspread_dB_Orig , PLabs_dB_Orig] = Trans2PL(f,d,transOrig);
disp(['PL-True Value: ',num2str(PLabs_dB_Orig)])
disp(['PL-Pred Value: ',num2str(PLabs_dB_Pred)])
disp(['PL-Error Value: ',num2str(abs(PLabs_dB_Pred-PLabs_dB_Orig))])
